...
namespace Signal_Source_Analyzer
{
    ...
    public class GeneralSpectrumAnalyzerSetup : SSAXBaseStep
    {
        ...
        [Display("Port 1", Group: "Attenuators", Order: 40)]
        [Unit("dB", UseEngineeringPrefix: true, StringFormat: "0")]
        public double Port1Attenuator { get; set; }

        [Browsable(false)]
        public bool IsPort2Enabled { get; set; } = false;
        [EnabledIf("IsPort2Enabled", true, HideIfDisabled = false)]
        [Display("Port 2", Group: "Attenuators", Order: 41)]
        [Unit("dB", UseEngineeringPrefix: true, StringFormat: "0")]
        public double Port2Attenuator { get; set; }
        ...
        public GeneralSpectrumAnalyzerSetup()
        {
            SweepPropertiesStart = 10e6;
            SweepPropertiesStop = 8e9;
            SweepPropertiesCenter = 4.0005e9;
            SweepPropertiesSpan = 7.99e9;
            SweepPropertiesPoints = 1001;

            ResolutionBandwidth = 1e6;
        ...
        public override void Run()
        {
            ...
            switch (SASweepType)
            {
                case SASweepTypeEnum.LinearFrequency:
                    SSAX.SetSASweepType(Channel, SASweepType);
                    if (IsStartStopCenterSpan == SweepSSCSTypeEnum.StartStop)
                    {
                        SSAX.SetStart(Channel, SweepPropertiesStart);
                        SSAX.SetStop(Channel, SweepPropertiesStop);
                    }
                    else
                    {
                        SSAX.SetCenter(Channel, SweepPropertiesCenter);
                        SSAX.SetSpan(Channel, SweepPropertiesSpan);
                    }
                    SSAX.SetPoints(Channel, SweepPropertiesPoints);
                    break;
            ...
            SSAX.SetSAResolutionBandwidth(Channel, ResolutionBandwidth);
            SSAX.SetSAResolutionBandwidthAuto(Channel, ResolutionBandwithAuto);
            SSAX.SetSAVideoBandwidth(Channel, VideoBandwidth);
            SSAX.SetSAVideoBandwidthAuto(Channel, VideoBandwithAuto);
            SSAX.SetSADetectorType(Channel, DetectorType);
            SSAX.SetSADetectorBypass(Channel, DetectorTypeBypass);
            SSAX.SetSAVideoAverageType(Channel, VideoAverageType);

            SSAX.SetSAPortAttenuation(Channel, 1, Port1Attenuator);
            SSAX.SetSAPortAttenuation(Channel, 2, Port2Attenuator);
        ...
        public override List<(string, object)> GetMetaData()
        {
            ...
            switch (SASweepType)
            {
                case SASweepTypeEnum.LinearFrequency:
                    ...
                    break;
                case SASweepTypeEnum.SegmentSweep:
                    // TODO add metadata for segment sweep
                    break;
            }
            ...
            retVal.Add(("SA Port 1 Attenuator", Port1Attenuator));
            retVal.Add(("SA Port 2 Attenuator", Port2Attenuator));
        ...
        public void SASetSegmentValues()
        {
            ...
            {
                SSAX.SegmentDeleteAllSegments(Channel);
                int segment = 0;
                foreach (SASegmentDefinition a in SAsegmentDefinitions)
                {
                    segment = SSAX.SegmentAdd(Channel);
                    SSAX.SetSegmentState(Channel, segment, a.state);
                    SSAX.SetSegmentNumberOfPoints(Channel, segment, a.NumberOfPoints);
                    SSAX.SetSegmentStartFrequency(Channel, segment, a.StartFrequency);
                    SSAX.SetSegmentStopFrequency(Channel, segment, a.StopFrequency);

                    //if (SASegmentAttributes.HasFlag(SASegmentAttributes.MultiToneRef))
                    //{
                    //    SSAX.SetSegmentSAMTReferenceControl(Channel, segment, SAOnOffTypeEnum.On);
                    //    SSAX.SetSegmentSAMTReference(Channel, segment, a.MTRef);
                    //}
                    //if (SASegmentAttributes.HasFlag(SASegmentAttributes.DataThreshold))
                    //{
                    //    SSAX.SetSegmentSADataThresholdControl(Channel, segment, SAOnOffTypeEnum.On);
                    //    SSAX.SetSegmentSADataThreshold(Channel, segment, a.DThreshold);
                    //}
                    //if (SASegmentAttributes.HasFlag(SASegmentAttributes.VectorAverage))
                    //{
                    //    SSAX.SetSegmentSAVectorAverageControl(Channel, segment, SAOnOffTypeEnum.On);
                    //    SSAX.SetSegmentSAVectorAverage(Channel, segment, a.VectAvg);
                    //}
                    if (SASegmentAttributes.HasFlag(SASegmentAttributes.VideoBandwidth))
                    {
                        SSAX.SetSegmentSAVideoBWControl(Channel, segment, SAOnOffTypeEnum.On);
                        SSAX.SetSegmentSAVideoBW(Channel, segment, a.VBW);
                    }
                }
                if (SAShowTable)
                {
                    SSAX.SetSegmentTableShow(Channel, true, 1);
                }
            }
