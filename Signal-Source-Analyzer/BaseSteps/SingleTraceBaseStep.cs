...
{
    [Browsable(false)]
    public class SingleTraceBaseStep : SSAXBaseStep
    {
        #region Settings
        [Browsable(false)]
        ...
        [Browsable(true)]
        [EnabledIf("IsFormatEnabled", true, HideIfDisabled = true)]
        [EnabledIf("EnableTraceSettings", true, HideIfDisabled = true)]
        ...
        {
            ChildTestSteps.Add(new TraceTitle() { SSAX = SSAX, Channel = Channel });
        }
        ...
        {
            ChildTestSteps.Add(new Marker() { SSAX = SSAX, Channel = Channel, mkr = NextMarker() });
        }
        ...
        {
            ChildTestSteps.Add(new TraceLimits() { SSAX = SSAX, Channel = Channel });
        }
        ...
        {
            ChildTestSteps.Add(new MultiPeakSearch() { SSAX = SSAX, Channel = Channel });
        }
        ...
        protected virtual void AddNewTraceToSSAX()
        {
            int _tnum = 0;
            int _mnum = 0;
            string _MeasName = "";
            ...
            //else
            //{
                SSAX.AddNewTrace(Channel, Window, Trace, measClass, finalMeasEnumName, ref _tnum, ref _mnum, ref _MeasName);
            //}
            tnum = _tnum;
            mnum = _mnum;
            MeasName = _MeasName;
