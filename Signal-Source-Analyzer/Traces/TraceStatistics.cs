...
namespace Signal_Source_Analyzer
{
    ...
    [Display("Statistics", Groups: new[] { "Signal Source Analyzer", "Trace" }, Description: "Set Statistics for a trace")]
    public class TraceStatistics : SSAXBaseStep
    {
        #region Settings
        ...
        public override void Run()
        {
            ...
            SSAX.MathStatistics(Channel, mnum, EnableStatistics);
            if (EnableStatistics)
            {
                SSAX.MathStatisticsRange(Channel, mnum, MathStatisticsRange);
                if (MathStatisticsRange != MathStatisticsRangeEnum.FullSpan)
                {
                    SSAX.MathStatisticsRangeStart(Channel, mnum, MathStatisticsRange, UserStart);
                    SSAX.MathStatisticsRangeStop(Channel, mnum, MathStatisticsRange, UserStop);
                }
                SSAX.MathShowResistance(Channel, mnum, ShowResistance);
            }
